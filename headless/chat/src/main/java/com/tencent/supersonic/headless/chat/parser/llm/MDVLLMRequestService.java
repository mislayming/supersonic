...
//
// import static
// com.tencent.supersonic.headless.chat.parser.ParserConfig.PARSER_LINKING_VALUE_ENABLE;
...
// List<Map.Entry<Long, Double>> schemaElementMatchMapList =
// schemaElementMatchMap.entrySet().stream()
// .map(e -> Map.entry(e.getKey(), e.getValue().stream().collect(Collectors.toMap(v ->
// v.getDetectWord(), Function.identity(), (o, n) ->
// o.getSimilarity() > n.getSimilarity() ? o :
// n)).values().stream().map(SchemaElementMatch::getSimilarity)
// .reduce(0D, Double::sum))).sorted(Comparator.comparingDouble(Map.Entry<Long,
// Double>::getValue).reversed())
...
// log.info("model=[{}] score=[{}] count=[{}]", modelId, entry.getValue(),
// currentSchemaElementMatches.size());
...
// element.getType().name(), element.getId(), element.getName(),
// currentSchemaElementMatch.getSimilarity(), currentSchemaElementMatch.getDetectWord());
...
// if (schemaElementMatchMapList.size() > 1 &&
// integrationProps.getSupersonic().getEnableLlmToMatchModel()) {
// // 查找模型名称
// List<Long> modelIdList =
// modelInfoList.stream().map(ModelMatchService.ModelInfo::getModelId).collect(Collectors.toList());
// if(CollectionUtil.isNotEmpty(modelIdList)) {
// Map<Long, ModelResp> modelMap = modelService.getModelList(new
// MetaFilter(modelIdList)).stream().collect(Collectors.toMap(ModelResp::getId, Function.identity(),
// (o, n) -> n));
...
// }
// });
// bestModelInfo = modelMatchService.findBestModel(queryCtx.getRequest().getQueryText(),
// modelInfoList);
// }
// }
// Long matchModelId = null;
// if (bestModelInfo != null &&
// schemaElementMatchMapList.stream().map(Map.Entry::getKey).collect(Collectors.toSet()).contains(bestModelInfo.getModelId()))
// {
...
// .filter(r -> r.getFromModelId().equals(currentModelId) ||
// r.getToModelId().equals(currentModelId))
// .collect(Collectors.toList());
// // 存在关联关系。
...
// */
// protected List<SchemaElement> getMappedMetrics(@NotNull ChatQueryContext queryCtx, Long
// dataSetId, Set<Long> matchModelList) {
// return super.getMappedMetrics(queryCtx, dataSetId).stream().filter(m ->
// matchModelList.contains(m.getModel())).collect(Collectors.toList());
// }
//
// protected List<SchemaElement> getMappedDimensions(@NotNull ChatQueryContext queryCtx, Long
// dataSetId, Set<Long> matchModelList) {
// return super.getMappedDimensions(queryCtx, dataSetId).stream().filter(d ->
// matchModelList.contains(d.getModel())).collect(Collectors.toList());
// }
//
// /***
...
// }
// return dimensionType != null && DimensionType.isTimeDimension(dimensionType) &&
// matchModelList.contains(d.getModel());
