...
public class SqlBuilder2 {
    ...
    private TableView render(OntologyQuery ontologyQuery, Set<DataModel> dataModels,
        ...
        for (DataModel dataModel : dataModels) {
            ...
            List<Dimension> allDimensions = ontology.getDimensionMap()
                    .getOrDefault(dataModel.getName(), Collections.emptyList());
            ...
            /*
             * for (Metric metric : allMetrics) { if
             * (ontologyQuery.getMetrics().contains(metric.getName())) { queryMetrics.add(metric); }
             * }
             */
    ...
    private SqlNode buildJoinTree(Map<String, TableView> tableViewMap, S2CalciteSchema schema,
        ...
        // 构建JOIN图
        Map<String, Set<JoinInfo>> joinGraph =
                buildJoinGraph(tableViewMap.keySet(), schema.getJoinRelations());
     ...
     * 构建JOIN关系图
     */
    private Map<String, Set<JoinInfo>> buildJoinGraph(Set<String> modelNames,
        ...
        for (JoinRelation relation : joinRelations) {
            if (modelNames.contains(relation.getLeft())
                    && modelNames.contains(relation.getRight())) {
     ...
     * 使用克鲁斯卡尔算法找出最小生成树边
     */
    private List<JoinInfo> getMSTEdges(Map<String, Set<JoinInfo>> joinGraph,
        ...
        // 定义查找根节点函数
        BiFunction<Map<String, String>, String, String> find =
                new BiFunction<Map<String, String>, String, String>() {
                    @Override
    ...
    private SqlNode executeJoinPlan(List<JoinInfo> joinEdges, Map<String, TableView> tableViewMap,
        ...
        for (JoinInfo joinInfo : joinEdges) {
            ...
            SqlNode joinNode = new SqlJoin(SqlParserPos.ZERO, leftNode, // 已经处理过别名
                    SqlLiteral.createBoolean(false, SqlParserPos.ZERO), joinType, rightNode, // 已经处理过别名
            ...
            for (String model : new HashSet<>(modelToGroup.keySet())) {
                if (modelToGroup.get(model).equals(leftGroup)
                        || modelToGroup.get(model).equals(rightGroup)) {
        ...
        if (sqlNodeMap.size() != 1) {
            log.error("JOIN树结果异常，预期1个节点，实际有{}个: {}", sqlNodeMap.size(),
                    String.join(", ", sqlNodeMap.keySet()));
        ...
        SqlNodeList selectItems =
                createSelectItemsForJoin(joinEdges, tableViewMap, modelOriginalAlias, scope);
    ...
    private SqlNodeList createSelectItemsForJoin(List<JoinInfo> joinEdges,
            Map<String, TableView> tableViewMap, Map<String, String> modelOriginalAlias,
                    ...
                    if (addedFields.contains(field)) {
                        ...
                        try {
                            ...
                            selectItems.add(new SqlBasicCall(SqlStdOperatorTable.AS,
                                    new SqlNode[] {columnRef, aliasNode}, SqlParserPos.ZERO));
                        } catch (Exception e) {
                            log.error("Error creating field reference for {}.{}: {}", alias, field,
                                    e.getMessage());
                            // 出错时使用原始方法作为回退
                            SqlNode[] nodes = new SqlNode[] {
                                            new SqlIdentifier(Arrays.asList(alias, field),
                                                    SqlParserPos.ZERO),
                                            new SqlIdentifier(alias + "_" + field,
                                                    SqlParserPos.ZERO)};
                            selectItems.add(new SqlBasicCall(SqlStdOperatorTable.AS, nodes,
                                    SqlParserPos.ZERO));
                        }
                    } else {
                        // 字段不存在，直接添加
                        try {
                            ...
                            log.error("Error creating field reference for {}.{}: {}", alias, field,
                                    e.getMessage());
                            // 出错时使用原始方法作为回退
                            selectItems.add(new SqlIdentifier(Arrays.asList(alias, field),
                                    SqlParserPos.ZERO));
                        }
        ...
        for (int i = 1; i < modelNames.size(); i++) {
            ...
            for (String processedModel : processedAliases.keySet()) {
                ...
                JoinRelation joinRelation =
                        findJoinRelation(processedModel, currentModel, schema.getJoinRelations());

                if (joinRelation != null
                        && !CollectionUtils.isEmpty(joinRelation.getJoinCondition())) {
                    ...
                    if (joinRelation.getLeft().equals(processedModel)) {
                        joinConditions =
                                joinRelation.getJoinCondition().stream()
                                        .map(con -> Triple.of(
                                                processedAliases.get(processedModel) + "."
                                                        + con.getLeft(),
                                                con.getMiddle(),
            ...
            if (condition == null) {
                ...
                constantOns.add(SqlLiteral.createExactNumeric("1", SqlParserPos.ZERO));
                condition = new SqlBasicCall(SqlStdOperatorTable.EQUALS, constantOns,
                        SqlParserPos.ZERO, null);
            }

            // 创建JOIN
            SqlLiteral joinType = SemanticNode.getJoinSqlLiteral("");
            result = new SqlJoin(SqlParserPos.ZERO, result,
     ...
     * 查找两个模型之间的JOIN关系
     */
    private JoinRelation findJoinRelation(String model1, String model2,
            List<JoinRelation> joinRelations) {
    ...
    @SneakyThrows
    public TableView renderOne(Set<Metric> queryMetrics, Set<Dimension> queryDimensions,
            DataModel dataModel, SqlValidatorScope scope, S2CalciteSchema schema) {
    ...
    private SqlNode createSafeFieldNode(String field, SqlValidatorScope scope) throws Exception {
        try {
            ...
            return SemanticNode.parse(field, scope,
    ...
    private Set<DataModel> probeRelatedModels(List<DataModel> dataModels, Ontology ontology) {
        ...
        // 收集所有查询涉及的模型名称
        Set<String> queryModelNames =
                dataModels.stream().map(DataModel::getName).collect(Collectors.toSet());
        ...
        Set<DataModel> result = neededModelNames.stream().filter(modelMap::containsKey)
                .map(modelMap::get).collect(Collectors.toSet());

        log.info("最终参与JOIN的模型: {}",
                result.stream().map(DataModel::getName).collect(Collectors.joining(", ")));
     ...
     * @return 所有需要的模型名称
     */
    private Set<String> findPathModels(Set<String> queryModels, Graph<String, DefaultEdge> graph,
            String modelType) {
        ...
        for (int i = 0; i < queryModelList.size(); i++) {
            for (int j = i + 1; j < queryModelList.size(); j++) {
                ...
                if (path != null) {
                    ...
                    log.debug("找到从{}到{}的路径，包含{}个模型", source, target, vertexList.size());
                }
            }
        }

        log.info("为查询模型找到{}条路径，共需要{}个{}", pathCount, pathModels.size(), modelType);
    ...
    private SqlNode buildJoinConditionFromTriples(List<Triple<String, String, String>> conditions,
        ...
        for (Triple<String, String, String> con : conditions) {
            ...
            SqlNode current = new SqlBasicCall(SemanticNode.getBinaryOperator(con.getMiddle()), ons,
                    SqlParserPos.ZERO, null);
            ...
            if (condition == null) {
                ...
                condition = new SqlBasicCall(SqlStdOperatorTable.AND,
                        new ArrayList<>(Arrays.asList(condition, current)), SqlParserPos.ZERO,
                        null);
            }
        }
