...
//    public static final String MDV_TABLE_NAME_PATTERN = String.format(MDV_TABLE_NAME_FORMATE, "(?<schemaName>(INTERFACE_SCHEMA_.{32}|SCHEMA_.{32}))");
//
//    @Override
...
//     * 最终SQL:
//     * (select t1.c1,t2.c2 from (select xxx from calciteSchemaXXX.real_t1) as t1 left join (select xxx from calciteSchemaYYY.real_t2) as t2 on t1.key = t2.key)
//     *
//     */
//    @SneakyThrows
...
//            SysSchemaFileBean schemaBeanInfo = SpringUtil.getBean(SchemaFileService.class).getSchemaBeanInfo(schemaName);
...
//            }
//            DBType dbType = null;
//            if(EResourceType.SCHEMA_V2.equals(schemaBeanInfo.getType()) || EResourceType.INTERFACE_SCHEMA.equals(schemaBeanInfo.getType())) {
//                dbType = DBType.MYSQL;
//            } else {
...
//                }
//                com.meicloud.mdv.model.DataSource dataSource = SpringUtil.getBean(DataSourceService.class).get(dataSourceId);
//                if (dataSource == null) {
//                    throw new NormalException("数据源ID不存在");
//                }
//                // 直连数据库，需要获取数据库类型。
...
//                } else {
//                    sqlQuery = CalciteUtils.appendDataSourceSchema(schemaBeanInfo.getDataSourceId(), schemaBean.getCubeBean().getServerName(), sqlQuery);
//                }
//            }
...
//            sql = sql.replaceAll(String.format("`%s`|%s", entry.getKey(), entry.getKey()), String.format("(%s) as %s", entry.getValue(), entry.getKey()));
//        }
...
//        stopWatch.stop();
//        log.info("LOGIC SQL TRANSLATE PHYSICS SQL TIME:[{}]ms", stopWatch.lastTaskInfo().getTimeMillis());
//        return sql;
//    }
//
//    /***
