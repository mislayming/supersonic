...
/**
 * 处理多数据源join的渲染器 主要职责是构建和处理多表join的SQL语句
 */
@Slf4j
public class JoinRender extends Renderer {
    ...
    public void render(OntologyQuery metricCommand, List<DataModel> dataModels,
        ...
        // 5. 构建join树
        SqlNode joinTree = buildJoinTree(joinGraph, dataModels, fieldWhere, measures, // queryMetrics
                queryAllDimension, // queryDimension
                queryWhere, scope, schema);
        ...
        for (Map.Entry<String, JoinRelation> entry : joinGraph.getJoinConditions().entrySet()) {
            ...
            if (joinRelation != null && !CollectionUtils.isEmpty(joinRelation.getJoinCondition())) {
                ...
                for (Triple<String, String, String> triple : joinRelation.getJoinCondition()) {
                    ...
                    // 添加左表字段
                    for (DataModel model : dataModels) {
                        if (model.getName().equals(leftTable)
                                && isFieldBelongToModel(triple.getLeft(), model)) {
                    ...
                    // 添加右表字段
                    for (DataModel model : dataModels) {
                        if (model.getName().equals(rightTable)
                                && isFieldBelongToModel(triple.getRight(), model)) {
    ...
    /**
     * 获取查询维度 从过滤维度中筛选出在查询维度或where条件字段中的维度
     */
    ...
    /**
     * 检查维度是否匹配 判断维度是否存在于源表维度、标识符或schema定义的维度中
     */
    ...
    /**
     * 获取匹配的Join关系 从schema中查找匹配的预定义join关系
     */
    ...
    /**
     * 添加zipper字段 为zipper类型的表添加时间范围字段
     */
    ...
    private SqlNode getCondition(TableView left, TableView right, DataModel dataModel,
        ...
        if (commonFields.isEmpty()) {
            ...
            leftFields.addAll(left.getDataModel().getDimensions().stream().map(Dimension::getName)
                    .collect(Collectors.toSet()));

            leftFields.addAll(left.getDataModel().getMeasures().stream().map(Measure::getName)
                    .collect(Collectors.toSet()));

            // 获取右表所有字段
            Set<String> rightFields = new HashSet<>();
            rightFields.addAll(right.getDataModel().getDimensions().stream().map(Dimension::getName)
                    .collect(Collectors.toSet()));
            rightFields.addAll(right.getDataModel().getMeasures().stream().map(Measure::getName)
                    .collect(Collectors.toSet()));
        ...
        for (String field : commonFields) {
            ...
            if (condition == null) {
                ...
                condition = new SqlBasicCall(SqlStdOperatorTable.AND,
                        new ArrayList<>(Arrays.asList(condition, joinCondition)), SqlParserPos.ZERO,
                        null);
            }
        }
    ...
    private SqlNode getCondition(JoinRelation joinRelation, SqlValidatorScope scope,
        ...
        for (Triple<String, String, String> triple : joinRelation.getJoinCondition()) {
            ...
            SqlNode joinCondition =
                    new SqlBasicCall(SemanticNode.getBinaryOperator(triple.getMiddle()), ons,
                            SqlParserPos.ZERO, null);
    ...
    private JoinGraph buildJoinGraph(List<DataModel> dataModels, S2CalciteSchema schema) {
        ...
        for (JoinRelation relation : schema.getOntology().getJoinRelations()) {
            ...
            boolean leftExists =
                    dataModels.stream().anyMatch(m -> m.getName().equals(relation.getLeft()));
            boolean rightExists =
                    dataModels.stream().anyMatch(m -> m.getName().equals(relation.getRight()));
    ...
    private SqlNode buildJoinTree(JoinGraph graph, List<DataModel> dataModels,
            ...
            String whereCondition, SqlValidatorScope scope, S2CalciteSchema schema)
            throws Exception {
        ...
        for (DataModel model : dataModels) {
            ...
            for (JoinRelation relation : graph.getJoinConditions().values()) {
                if (relation.getLeft().equals(model.getName())) {
                    neededFields.addAll(relation.getJoinCondition().stream().map(Triple::getLeft)
                            .collect(Collectors.toSet()));
                }
                if (relation.getRight().equals(model.getName())) {
                    neededFields.addAll(relation.getJoinCondition().stream().map(Triple::getRight)
                            .collect(Collectors.toSet()));
                }
            }
            ...
            TableView tableView = buildTableView(model, whereFields, queryMetrics, queryDimension,
                    whereCondition, scope, schema, neededFields // 传入所有需要的字段
            );
        ...
        List<String> joinOrder =
                findOptimalJoinOrderRelaxed(dataModels, graph, queryMetrics, queryDimension);

        // 3. 按照优化后的顺序构建join树
        String firstTable = joinOrder.get(0);
        SqlNode currentNode = SemanticNode.buildAs(tableViews.get(firstTable).getAlias(),
        ...
        for (int i = 1; i < joinOrder.size(); i++) {
            ...
            if (joinRelation != null && !CollectionUtils.isEmpty(joinRelation.getJoinCondition())) {
                // 使用预定义的join关系
                condition =
                        getCondition(joinRelation, scope, schema.getOntology().getDatabaseType());
                joinType = SemanticNode.getJoinSqlLiteral(joinRelation.getJoinType());
            } else {
                // 如果图中没有预定义关系，使用标识符作为join条件
                condition = getCondition(leftView, rightView, rightView.getDataModel(), schema,
                        scope, schema.getOntology().getDatabaseType());
                joinType = SemanticNode.getJoinSqlLiteral("");
            }

            // 构建join节点
            SqlNode joinNode = new SqlJoin(SqlParserPos.ZERO, currentNode,
    ...
    private String findBestJoinTable(String currentTable, List<String> addedTables,
            JoinGraph graph) {
    ...
    private List<String> findOptimalJoinOrderRelaxed(List<DataModel> dataModels, JoinGraph graph,
        ...
        for (DataModel model : dataModels) {
            int weight = 0;
            weight += model.getMeasures().stream().filter(m -> queryMetrics.contains(m.getName()))
                    .count() * 2;
            weight += model.getDimensions().stream()
                    .filter(d -> queryDimension.contains(d.getName())).count();
    ...
    private TableView buildTableView(DataModel dataModel, List<String> whereFields,
            ...
            SqlValidatorScope scope, S2CalciteSchema schema, Set<String> joinFields)
            throws Exception {
        ...
        // 1.1 添加where字段
        neededFields
                .addAll(whereFields.stream().filter(field -> isFieldBelongToModel(field, dataModel))
                        .collect(Collectors.toList()));

        // 1.2 添加查询度量
        neededFields.addAll(
                queryMetrics.stream().filter(metric -> isMetricBelongToModel(metric, dataModel))
                        .collect(Collectors.toSet()));

        // 1.3 添加查询维度
        neededFields.addAll(
                queryDimension.stream().filter(dim -> isDimensionBelongToModel(dim, dataModel))
                        .collect(Collectors.toSet()));

        // 1.4 添加用于join的字段
        neededFields
                .addAll(joinFields.stream().filter(field -> isFieldBelongToModel(field, dataModel))
                        .collect(Collectors.toSet()));
        ...
        TableView tableView = SourceRender.renderOne("", // prefix
        ...
        public void addEdge(String table1, String table2, JoinRelation relation) {
            ...
            if (relation.getJoinCondition() != null) {
                for (Triple<String, String, String> condition : relation.getJoinCondition()) {
                    log.debug("Join condition: {}.{} {} {}.{}", table1, condition.getLeft(),
                            condition.getMiddle(), table2, condition.getRight());
                }
            }
        ...
        public String toString() {
            ...
            for (Map.Entry<String, Set<String>> entry : adjacencyList.entrySet()) {
                ...
                for (String table2 : entry.getValue()) {
                    String pairKey = table1.compareTo(table2) < 0 ? table1 + "_" + table2
                            : table2 + "_" + table1;
                    ...
                    if (!printedPairs.contains(pairKey)) {
                        ...
                        if (relation != null && relation.getJoinCondition() != null) {
                            for (Triple<String, String, String> condition : relation
                                    .getJoinCondition()) {
                                sb.append(String.format("    %s.%s %s %s.%s\n", table1,
                                        condition.getLeft(), condition.getMiddle(), table2,
                                        condition.getRight()));
                            }
                        }
